...
class SiameseModelTTA(nn.Module):
    ...
    def forward(self, img):
        with torch.cuda.amp.autocast(True):
            ...
            out = out.float()
            out = self.act(out).reshape(4, -1, *out.shape[-2:])
            ret = out[0]
...
def get_models(names, folds):
    ...
    for model_info in model_infos:
        ...
        model.eval()
        model.cuda()
        models.append([model, model_info["weight"]])
    return models
...
with torch.no_grad():
    ...
    for imgA, imgB in tqdm(dl):
        ...
        for pred in preds:
            pool.apply_async(get_dt, args = (pred, sub.loc[idx, "uid"], dts, thres_list))
            idx += 1
total_time += time.time()
