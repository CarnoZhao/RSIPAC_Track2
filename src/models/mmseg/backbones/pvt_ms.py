...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., linear=False):
        ...
        if isinstance(self.act, nn.GELU):
            self.act = ms.GELU(quant_groups=hidden_features // GROUP_SIZE)
        ...
        if self.linear:
            ...
            if isinstance(self.relu, nn.ReLU):
                self.act = ms.ReLU(quant_groups = out_features // GROUP_SIZE)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, linear=False):
        ...
        if not linear:
            ...
            self.act = nn.GELU()
            if isinstance(self.act, nn.GELU):
                self.act = ms.GELU(quant_groups=dim // 32)
        self.apply(self._init_weights)
    ...
    def forward(self, x, H, W):
        ...
        attn = self.softmax(attn) # .softmax(dim=-1)
...
class PyramidVisionTransformerV2_(nn.Module):
    def __init__(self, 
        ...
        for i in range(num_stages):
            ...
            self.out_channels.append(embed_dims[i])
            cur += depths[i]
...
class PyramidVisionTransformerV2MS(PyramidVisionTransformerV2_):
    def __init__(self, model_name = None, pretrained = None, policy_path = None, **kwargs):
        if model_name is None:
            super(PyramidVisionTransformerV2MS, self).__init__(**kwargs)
        ...
        ms.policy.convert_by_group_size(self, GROUP_SIZE)
        ms.policy.deploy_on_init(self, policy_path, verbose=print, override_verbose=False)
