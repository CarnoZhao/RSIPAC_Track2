...
class ConvRelPosEnc(nn.Module):
    ...
    def forward(self, q, v, size):
        ...
        v_img = v_img.reshape(B, h, H, W, Ch).permute(0, 1, 4, 2, 3).reshape(B, h * Ch, H, W)
        ...
        conv_v_img = conv_v_img.reshape(B, h, Ch, H, W).permute(0, 1, 3, 4, 2).reshape(B, h, H * W, Ch)
...
class CoaT(CoaT_):
    def __init__(self, model_name = None, pretrained = None, **kwargs):
        if model_name is None:
            super(CoaT, self).__init__(**kwargs)
        elif model_name in ("tiny", "mini", "small", "lite_tiny", "lite_mini", "lite_small", "lite_medium"):
            kwargs["patch_size"] = 4
            kwargs["embed_dims"] = {
                "tiny": [152, 152, 152, 152],
                "mini": [152, 216, 216, 216],
                "small": [152, 320, 320, 320],
                "lite_tiny": [64, 128, 256, 320],
                "lite_mini": [64, 128, 320, 512],
                "lite_small": [64, 128, 320, 512],
                "lite_medium": [128, 256, 320, 512]
            }[model_name]
            kwargs["serial_depths"] = {
                "lite_small": [3, 4, 6, 3],
                "lite_medium": [3, 6, 10, 8],
            }.get(model_name, [2, 2, 2, 2])
            kwargs["parallel_depth"] = 0 if "lite" in model_name else 6
            kwargs["num_heads"] = 8
            kwargs["mlp_ratios"] = [8, 8, 4, 4] if "lite" in model_name and "medium" not in model_name else [4, 4, 4, 4]
            super(CoaT, self).__init__(**kwargs)
        else:
