...
from ..conv_module import build_norm_layer
...
class Stem(nn.Module):
    def __init__(self, in_channels, stem_hidden_dim, out_channels, norm_cfg = dict(type='BN', requires_grad=True)):
        super().__init__()
...
class DualViT_(nn.Module):
    def __init__(self,  
        ...
        for i in range(self.num_stages):
            if i == 0:
                patch_embed = Stem(in_chans, stem_hidden_dim, embed_dims[i], norm_cfg)
            else:
...
class DualViT(DualViT_):
    def __init__(self, model_name = None, pretrained = None, **kwargs):
        if model_name is None:
            super(DualViT, self).__init__(**kwargs)
        elif model_name in ("small", "base", "large"):
            
            kwargs["stem_hidden_dim"] = {
                "small": 32,
                "base": 64,
                "large": 64,
            }[model_name]
            kwargs["embed_dims"] = {
                "small": [64, 128, 320, 448],
                "base": [64, 128, 320, 512],
                "large": [96, 192, 384, 512],
            }[model_name]
            kwargs["num_heads"] = {
                "small": [2, 4, 10, 14],
                "base": [2, 4, 10, 16],
